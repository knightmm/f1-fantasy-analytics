...
def make_constructors_dataframe(df):
    constructor_filter = df["asset_type"] == "CONSTRUCTOR"
    constructors_df = df[constructor_filter].copy()

    constructor_drop_cols = [
        "team_name",
        "status",
        "driver_reference",
        "country_name",
    ...
    return constructors_df.drop(columns=constructor_drop_cols, errors="ignore")


def make_drivers_dataframe(df):
    driver_filter = df["asset_type"] == "DRIVER"
